...
def get_job_specs(es_url, logger=None, job_spec_type=JOB_SPEC_TYPE):
    '''
...
def get_job_spec(es_url, ident, logger=None, job_spec_type=JOB_SPEC_TYPE):
    '''
...
def add_job_spec(es_url, obj, logger=None, job_spec_type=JOB_SPEC_TYPE):
    '''
...
def remove_job_spec(es_url, ident, logger=None, job_spec_type=JOB_SPEC_TYPE):
    '''
    Remove a container
    @param es_url - elasticsearch URL
    @param job_spec_type - es index type, now _doc type in es7+, preserve for now
    ...
    '''
    return hysds_commons.metadata_rest_utils.remove_metadata(es_url, JOB_SPEC_INDEX, job_spec_type, ident,
                                                             logger=logger)
