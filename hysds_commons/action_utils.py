import os, sys, operator, functools, json, traceback
...
def create_action_from_entry(jobspec_es_url, wiring, ops_account="ops"):
    ...
    try:
        ...
        action["kwargs"] = [param for param in wiring.get("params", []) if param.get("from", "") == "submitter"]
        ...
        action["passthru_query"] = functools.reduce(
            lambda x, param: operator.or_(x, param["from"] == "passthrough" and param["name"] == "query"),
        ...
        if "label" in wiring:
            action["label"] = "{0} [{1}]".format(wiring.get("label"), wiring.get("job-version", "unknown-version"))
        else:
...
def get_action_spec(iospec_es_url, jobspec_es_url, ops_account="ops"):
    ...
    action_specs = []
    wirings = [spec.get('_source', {}) for spec in hysds_commons.hysds_io_utils.get_hysds_ios(iospec_es_url)]
    ...
    for wiring in wirings:
        if not "job-specification" in wiring or not wiring["job-specification"] in jobs:
            continue
        action_specs.append(create_action_from_entry(jobspec_es_url, wiring, ops_account))
